# APP SKILL.md 草稿（B1 施工底稿）

> **用途**：本文件是 `.agents/skills/app/SKILL.md` 的全文草稿，逐节对照 `.agents/skills/xcx/SKILL.md`（242 行，canonical）改写。B1 施工时以此为底稿落成正式文件，并复制到 `.claude/skills/app/`、`.opencode/skills/app/` 镜像（`python scripts/check_skill_drift.py` 必须为 0）。硬约束 0-9 与 AI 结论模板为项目级纪律，与 xcx 保持逐字一致；差异集中在流程主体（游标文件、平台语义、设备/脱壳审批门、台账清单、audit 命令）。
> **对照基准**：xcx SKILL.md @ 当前 canonical（2026-09-07 读取版本）。
...
---
name: app
description: Advance phases of an authorized mobile app (Android APK/XAPK, iOS ipa) assessment. After each phase, write the cursor and handoff-complete records to disk, then ASK the operator whether to continue in this session or hand off — if handoff, emit a self-contained prompt for the new session. Sessions still hard-stop at approval-gated phases (device instrumentation, hardened unpack, exploitability), heavy phases (dynamic_setup, report), or the context budget ladder. Start from an APK/IPA file, package name, market link, unpacked directory, traffic export, or a specified phase. Use when an AI must push the current phase of a mobile app — not assess it completely in one sitting.
---
...
5. **Raw artifacts stay on disk.** Responses, HAR, JS, decompiled source, or scan output never enter the conversation — cite `path:line` only.
...
8. **Context budget ladder（2026-08-23；绝对 token，换模型不失效）.** (a) *Recommend-handoff line* — ~120K tokens（重推理阶段：review verdicts/planning/复杂调试）或 ~150K（轻量脚本阶段：init/package_inventory/static_analysis），在阶段边界推荐交接；(b) *Must-wrap line* — min(200K, 窗口 70%)，立即收尾写盘并交接。当前 1M 窗口换算：约 12-15% 与 20%。

9. **Refuse end-to-end requests.** If the operator asks to "do the whole app in one session", decline and explain the session-window rule: phases advance under constraint 1, and approval-gated or heavy phases always stop.
...
## 与 xcx SKILL 的差异清单（B1 校对用）

| 节 | 差异 |
|---|---|
| frontmatter description | 换为移动应用语境（APK/IPA/包名/市场链接），审批门列举 device_instrumentation/app_hardened_unpack |
| 约束 1 硬停列举 | 审批门加 device_instrumentation / app_hardened_unpack；重量级换 dynamic_setup / dynamic_mapping / reporting |
| 约束 2(a) | target-model 增加 exported components/deeplinks 入口面 |
| 约束 2(c) operator_tasks | 设备准备/脱壳材料供给/设备改动清理 |
| 约束 2(d) | `init_app_engagement.py` |
| 细微发现处置 | 换移动端信号清单（壳特征/manifest 权限/exported 组件/allowBackup/调试开关等） |
| 主体 1-9 | 平台识别 android/ios/dual；FairPlay→blocked；设备/加固双钥匙节（第 9 条，新增） |
| 工具发现节 | jadx/apktool/whitebox_triage 受管纪律 + 壳三分支不塌缩 |
| Execute one phase | `phase_status.app.json` 三流隔离句 |
| 台账清单 | `app.json`、`artifacts/app/*` 路径 |
| Validate and close | audit_app_engagement.py；closed 条件含设备改动清理 |
| Burp 节 | 设备语境；注入/脱壳/写型动作审批门 |
...
> 落地文件：`prompts/配方APP_App流程.md`；结构对照 `prompts/配方XCX_小程序流程.md`（56 行）。
